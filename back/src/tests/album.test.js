...
describe("ALBUM", () => {
    ...
    test("Criar album, bem sucedido", async () => {
      ...
      jest
        ...
        .mockImplementationOnce(() =>
          Promise.resolve({
            ...JSON.parse(JSON.stringify(mockedAlbum)),
            save: () => {},
            songs: [],
          })
        );
  ...
  describe("PUT /albums/:albumId", () => {
    ...
    test("falha ao editar album, album não existe", async () => {
      ...
      const results = await supertest(app)
        .put("/albums/" + mockedAlbumCreated._id)
        .send()
        .set("Authorization", "Bearer " + token);
    ...
    test("falha ao editar album, nome já existe", async () => {
      mockingoose(Album).toReturn(
        { ...mockedAlbumCreated, _id: "diferente" },
        "findOne"
      );
      const results = await supertest(app)
        .put("/albums/" + mockedAlbumCreated._id)
    ...
    test("editar album, sucesso", async () => {
      const mockedAlbumEdited = {
        ...
        year: 2020,
        image:
          "https://wompampsupport.azureedge.net/fetchimage?siteId=7575&v=2&jpgQuality=100&width=700&url=https%3A%2F%2Fi.kym-cdn.com%2Fphotos%2Fimages%2Fnewsfeed%2F001%2F472%2F186%2F337.jpg",
        artist: "507f191e810c19729de860ea",
        songs: [
          {
            name: "Banjo Boy",
